...
//                        results : results,
    ...
    orderDetails: function (req, res) {
        ...
        models.orders.findOne({
            ...
            include: [{
                    attributes: ['id', 'price', 'quantity', 'statusId',
                        [models.sequelize.fn('MIN', models.sequelize.col('orderitems.statusId')), 'statusMin1']
                    ],
                    model: models.orderitems,
                    required: false,
                }, {
                    attributes: ['id', 'price', 'quantity', 'statusId',
                        [models.sequelize.fn('MIN', models.sequelize.col('orderdealsitems.statusId')), 'statusMin2']
                    ],
                    model: models.orderdealsitems,
                    required: false,
                }]
                ...
                var minval;
                    if(typeof data.orderitems[0] != 'undefined'  &&   data.orderitems.length > 0 && typeof data.orderdealsitems[0] != 'undefined' && data.orderdealsitems.length > 0){
                       if (data.orderitems[0].get('statusMin1') < data.orderdealsitems[0].get('statusMin2')) {
                         minval = data.orderitems[0].get('statusMin1')
                     }else{
                         minval = data.orderdealsitems[0].get('statusMin2')
                    }     
                           }else if( typeof data.orderitems[0] == 'undefined'  ){
                             minval = data.orderdealsitems[0].get('statusMin2')
                               }else if(typeof data.orderdealsitems[0] == 'undefined' ){
                                  minval = data.orderitems[0].get('statusMin1')
                    }
